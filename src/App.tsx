...
function App() {
  ...
  const [accounts, setAccounts] = useState<Account[]>(() => { try { return JSON.parse(localStorage.getItem('rahnegar-users') || '[{"username":"شرکت گاز","password":"123456789"}]') } catch { return [{username:'شرکت گاز',password:'123456789'}] } })
  ...
  const [view, setView] = useState<'roadmap'|'calendar'|'archive'|'capabilities'|'history'|'settings'>('roadmap')
  ...
  const visible = useMemo(() => items.filter(i =>
    ...
    (ownerFilter === 'همه تیم‌ها' || i.owner === ownerFilter) && (!urgentOnly || i.urgent) && Number((i.entryYear||currentYear).replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d))))===selectedYear
  ...
  const login = (e: React.FormEvent) => {
    ...
    setLoginError(''); setUser({name:loginName.trim(),role:savedProfiles[loginName.trim()]?.role||'',email:savedProfiles[loginName.trim()]?.email||''}); localStorage.setItem('rahnegar-current-user',loginName.trim()); localStorage.setItem('rahnegar-login','gas-company-authenticated'); setLoggedIn(true)
  }

  const addAccount = () => {
    if (user.name !== 'شرکت گاز' || !newAccount.username.trim() || newAccount.password.length < 4 || accounts.some(a=>a.username===newAccount.username.trim())) return
    const next=[...accounts,{username:newAccount.username.trim(),password:newAccount.password}]; setAccounts(next); localStorage.setItem('rahnegar-users',JSON.stringify(next)); setNewAccount({username:'',password:''})
  }
  ...
  return <div className="app-shell" dir="rtl">
    <aside className={`sidebar ${sidebarOpen?'open':'collapsed'}`}>
      ...
      <button className="sidebar-foot" onClick={()=>setProfileOpen(true)}><div className="avatar">{user.name.slice(0,2)}</div><div><b>{user.name}</b><span>{user.role}</span></div><MoreHorizontal size={18}/></button>
    </aside>
    ...
    <main className={sidebarOpen?'sidebar-expanded':'sidebar-collapsed'}>
      <header className="topbar">
        ...
        <div className="breadcrumb">نقشه راه تحول فناوری گاز استان مرکزی <span>/</span> <b>{view==='calendar'?'تقویم شمسی':view==='archive'?'آرشیو نسخه‌ها':view==='capabilities'?'توانمندی واحدها':view==='history'?'تاریخچه تغییرات':view==='settings'?'تنظیمات':dashboard === 'tool' ? 'فناوری‌های ابزاری' : 'فناوری‌های قابل توسعه'}</b></div>
        <div className="top-actions"><div className="live-date"><span>امروز</span><b>{today}</b></div><label className="year year-selector"><CalendarDays size={17}/><span>سال</span><select value={selectedYear} onChange={e=>{setSelectedYear(+e.target.value);setCalendarMonth(0)}}>{Array.from({length:currentYearNumber-1360+2},(_,i)=>currentYearNumber+1-i).map(y=><option key={y} value={y}>{fa(y)}</option>)}</select><ChevronDown size={15}/></label></div>
      </header>

      <section className="content">
        {view === 'calendar' && <div className="calendar-page">
          <div className="page-title"><div><h1>تقویم شمسی رویدادهای فناوری</h1><p>نمای یکپارچه برنامه‌ها و رویدادهای نقشه راه در سال {fa(selectedYear)}</p></div><div className="calendar-nav"><button onClick={()=>setCalendarMonth(Math.max(0,calendarMonth-1))}>ماه قبل</button><b>{months[calendarMonth]} {fa(selectedYear)}</b><button onClick={()=>setCalendarMonth(Math.min(11,calendarMonth+1))}>ماه بعد</button></div></div>
          <div className="calendar-layout"><div className="persian-calendar"><div className="weekdays">{['شنبه','یکشنبه','دوشنبه','سه‌شنبه','چهارشنبه','پنجشنبه','جمعه'].map(d=><span key={d}>{d}</span>)}</div><div className="days">{Array.from({length:31},(_,i)=>i+1).map(day=>{const dayEvents=visible.filter(it=>monthDay(it.start).month===calendarMonth&&monthDay(it.start).day===day);return <div className={`day ${dayEvents.length?'has-event':''}`} key={day}><b>{fa(day)}</b>{dayEvents.slice(0,3).map(it=><button key={it.id} style={{borderRightColor:it.color}} onClick={()=>setSelected(it)}>{it.title}<small>{it.status}</small></button>)}</div>})}</div></div><aside className="calendar-agenda"><h3>رویدادهای {months[calendarMonth]}</h3><p>{fa(visible.filter(i=>monthDay(i.start).month===calendarMonth).length)} مورد ثبت شده</p>{visible.filter(i=>monthDay(i.start).month===calendarMonth).sort((a,b)=>a.start-b.start).map(it=><button key={it.id} onClick={()=>setSelected(it)}><i style={{background:it.color}}/><span><b>{it.title}</b><small>{fa(monthDay(it.start).day)} {months[calendarMonth]} · {it.owner||'بدون مسئول'}</small></span></button>)}</aside></div>
        </div>}
        ...
        {view === 'settings' && <div className="settings-page"><div className="page-title"><div><h1>تنظیمات داشبورد</h1><p>شخصی‌سازی ظاهر، نمایش، اعلان‌ها و مدیریت داده‌ها</p></div></div><div className="settings-grid"><section><header><span><Eye/></span><div><h3>ظاهر داشبورد</h3><p>رنگ و نحوه نمایش رابط کاربری</p></div></header><div className="theme-options"><button className={theme==='light'?'selected':''} onClick={()=>setTheme('light')}><i className="light-preview"/><b>تم روشن</b><small>پس‌زمینه روشن و خوانا</small></button><button className={theme==='dark'?'selected':''} onClick={()=>setTheme('dark')}><i className="dark-preview"/><b>تم تاریک</b><small>مناسب محیط کم‌نور</small></button></div><label className="setting-row"><span><b>نمایش فشرده</b><small>فاصله کمتر بین آیتم‌ها و جدول‌ها</small></span><input type="checkbox" checked={compactMode} onChange={e=>setCompactMode(e.target.checked)}/></label><label className="setting-row"><span><b>جلوه‌های حرکتی</b><small>انیمیشن بازشدن پنل‌ها و منو</small></span><input type="checkbox" checked={animations} onChange={e=>setAnimations(e.target.checked)}/></label></section><section><header><span><Settings/></span><div><h3>تنظیمات عمومی</h3><p>رفتار پیش‌فرض داشبورد</p></div></header><label className="settings-field">سال پیش‌فرض<select value={selectedYear} onChange={e=>setSelectedYear(+e.target.value)}>{Array.from({length:currentYearNumber-1360+2},(_,i)=>currentYearNumber+1-i).map(y=><option key={y}>{y}</option>)}</select></label><label className="settings-field">نمای پیش‌فرض<select value={zoom} onChange={e=>setZoom(e.target.value)}><option>ماهانه</option><option>فصلی</option><option>سالانه</option></select></label><label className="setting-row"><span><b>اعلان تغییرات</b><small>نمایش پیام پس از ذخیره و ویرایش</small></span><input type="checkbox" checked={notifications} onChange={e=>{setNotifications(e.target.checked);localStorage.setItem('rahnegar-notifications',e.target.checked?'yes':'no')}}/></label></section><section><header><span><Archive/></span><div><h3>داده و نگهداری</h3><p>وضعیت اطلاعات ذخیره‌شده</p></div></header><div className="storage-info"><div><b>{fa(items.length)}</b><small>آیتم فناوری</small></div><div><b>{fa(snapshots.length)}</b><small>نسخه تثبیت‌شده</small></div><div><b>{fa(history.length)}</b><small>رکورد تاریخچه</small></div></div><p className="storage-note">تمام اطلاعات در مرورگر ذخیره می‌شوند و پس از خروج از حساب باقی می‌مانند.</p></section></div></div>}
        {view === 'roadmap' && <>
        ...
        <div className="roadmap-card">
          ...
          <div className="timeline-scroll">
            <div className={`timeline zoom-${zoom === 'ماهانه' ? 'month' : zoom === 'فصلی' ? 'quarter' : 'year'}`}>
              ...
              {lanes.map(lane => <div className="swim-row" key={lane.id} style={{background:lane.tint}}>
